...
export default function RetentionBanner({ isAdmin }: RetentionBannerProps) {
  if (isAdmin) {
    return (
      <div className="w-full bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 flex items-center justify-between text-xs text-muted-foreground shadow-sm">
        <div className="flex items-center space-x-3">
          <div className="p-2 rounded-xl bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30 shrink-0">
        ...
        <span className="hidden sm:inline font-mono bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 border border-emerald-500/30 px-2.5 py-1 rounded-lg text-[10px] font-bold">
          PERMANENT TTL
        </span>
      </div>
    );
  ...
  return (
    <div className="w-full bg-card/70 border border-amber-500/30 rounded-2xl p-4 flex items-center justify-between text-xs text-muted-foreground shadow-sm">
      <div className="flex items-center space-x-3">
        <div className="p-2 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/30 shrink-0">
      ...
      <span className="hidden sm:inline font-mono bg-secondary text-amber-600 dark:text-amber-400 border border-amber-500/30 px-2.5 py-1 rounded-lg text-[10px] font-semibold">
        30-DAY LIFECYCLE
      </span>
    </div>
  );
